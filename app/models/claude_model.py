...
class ClaudeModel(AIModelBase):
    """Claude model for SQL generation using direct API calls"""
    
    model_type = 'claude'
    
    def __init__(self):
        """Initialize the Claude model with direct HTTP requests"""
        super().__init__()
        self.api_key = current_app.config.get('CLAUDE_3_OPUS_API_KEY')
        self.model = current_app.config.get('CLAUDE_3_OPUS_MODEL', 'claude-3-opus-20240229')
        self.api_url = "https://api.anthropic.com/v1/messages"
        self.initialized = False
        
        # Check if API key is configured
        if not self.api_key:
            logger.warning("Claude API key not configured. Please set CLAUDE_3_OPUS_API_KEY in your environment variables.")
            return
        ...
        try:
            ...
            logger.info("Claude API integration initialized successfully using direct HTTP requests")
        except Exception as e:
            logger.warning(f"Error initializing Claude API integration: {str(e)}")
    ...
    def generate_sql(self, prompt: str, schema_data: Dict[str, Any], db_type: str, db_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Generate SQL query using Claude API with direct HTTP requests"""
        if not self.initialized:
            return {
                'success': False,
                'error': "Claude API not properly initialized",
...
{prompt}"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                sql_query = content[0].get('text', '').strip() if content else ''
    ...
    def analyze_explain_plan(self, explain_data: Dict[str, Any], db_type: str, sql_query: str) -> Dict[str, Any]:
        """Analyze database explain plan and provide recommendations using Claude API"""
        if not self.initialized:
            return {
                'success': False,
                'error': "Claude API not properly initialized",
...
7. Structure your analysis in a clear, organized manner."""
...
4. An overall efficiency rating (1-10)"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                content = result.get('content', [])
                analysis_text = content[0].get('text', '').strip() if content else ''
            ...
            # Handle exceptions
            logger.error(f"Error analyzing explain plan with Claude: {str(e)}")
            return {
                'success': False,
