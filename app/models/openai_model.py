...
class OpenAIModel(AIModelBase):
    """OpenAI GPT model for SQL generation using direct HTTP requests"""
    
    model_type = 'openai'
    
    def __init__(self):
        """Initialize the OpenAI model with direct HTTP requests"""
        super().__init__()
        ...
        if not self.api_key:
            logger.warning("OpenAI API key not configured. Please set OPENAI_API_KEY in your environment variables.")
            return
        ...
        try:
            ...
            logger.info("OpenAI API integration initialized successfully using direct HTTP requests")
        except Exception as e:
            logger.warning(f"Error initializing OpenAI API integration: {str(e)}")
    ...
    def generate_sql(self, prompt: str, schema_data: Dict[str, Any], db_type: str, db_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Generate SQL query using OpenAI API with direct HTTP requests"""
        if not self.initialized:
            return {
                'success': False,
                'error': "OpenAI API not properly initialized",
...
{prompt}"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if sql_query.startswith('```') and '```' in sql_query[3:]:
                    ...
                    first_delimiter = sql_query.find('```')
                    second_delimiter = sql_query.find('```', first_delimiter + 3)
                    if second_delimiter > first_delimiter:
                        # Check if there's a language identifier (like 'sql')
                        language_end = sql_query.find('\n', first_delimiter + 3)
                        if language_end > first_delimiter and language_end < second_delimiter:
                            # Extract content between first newline after opening ``` and closing ```
                            sql_query = sql_query[language_end + 1:second_delimiter].strip()
                        else:
                            # Extract content between opening and closing ```
                            sql_query = sql_query[first_delimiter + 3:second_delimiter].strip()
    ...
    def analyze_explain_plan(self, explain_data: Dict[str, Any], db_type: str, sql_query: str) -> Dict[str, Any]:
        """Analyze database explain plan and provide recommendations using OpenAI API"""
        if not self.initialized:
            return {
                'success': False,
                'error': "OpenAI API not properly initialized",
...
4. An overall efficiency rating (1-10)"""
        ...
        try:
            ...
            # Handle exceptions
            logger.error(f"Error analyzing explain plan with OpenAI: {str(e)}")
            return {
                'success': False,
